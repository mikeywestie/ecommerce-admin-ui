...
function ReadmeTab() {
  return (
    <div className="space-y-6 text-sm leading-6 text-slate-300">
      ...
      <section><h3 className="text-lg font-bold text-white">Project links</h3><div className="mt-3 grid gap-3 sm:grid-cols-3"><ProjectLink label="Frontend Repo" /><ProjectLink label="Backend Repo" /><ProjectLink label="Portfolio Website" /></div></section>
    </div>
  );
}
...
function ArchitectureTab() {
  return (
    <div className="space-y-6 text-sm leading-6 text-slate-300">
      <section><h3 className="text-lg font-bold text-white">High-level architecture</h3><div className="mt-3 rounded-2xl border border-slate-700 bg-slate-950/60 p-5 font-mono text-sm text-slate-200"><p>React Frontend</p><p className="text-blue-300">      |</p><p className="text-blue-300">      v</p><p>Spring Boot REST API</p><p className="text-blue-300">      |</p><p className="text-blue-300">      v</p><p>PostgreSQL Database</p></div></section>
      <section><h3 className="text-lg font-bold text-white">Architecture highlights</h3><div className="mt-3 grid gap-3 sm:grid-cols-2"><InfoPill label="Security" value="JWT auth + role-based access" /><InfoPill label="API" value="REST endpoints for admin and customer flows" /><InfoPill label="Monitoring" value="Spring Boot Actuator health checks" /><InfoPill label="Deployment" value="Frontend and backend deployed separately" /></div></section>
      <section><h3 className="text-lg font-bold text-white">User flows</h3><ul className="mt-3 list-disc space-y-2 pl-5"><li>Admin users can view dashboards, orders, payments, coupons, inventory, and system health.</li><li>Customer users can browse products, add items to cart, checkout, and view orders.</li><li>The login screen selects demo credentials and redirects users based on their authenticated role.</li></ul></section>
    </div>
  );
}
...
function ReleaseNotesTab() {
  return (
    <div className="space-y-6 text-sm leading-6 text-slate-300">
      <section className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4"><span className="rounded-full border border-emerald-500/40 bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-300">Active Development</span><h3 className="mt-4 text-lg font-bold text-white">Project status</h3><p className="mt-2">Current version: 1.0.0</p><p>Last updated: June 2026</p></section>
      <section><h3 className="text-lg font-bold text-white">Latest changes</h3><ul className="mt-3 list-disc space-y-2 pl-5"><li>Added customer and admin demo account selector.</li><li>Added live API health badge on the login screen.</li><li>Added customer storefront flow with products, cart, checkout, and orders.</li><li>Added admin modules for dashboard, inventory, payments, orders, coupons, and system health.</li><li>Expanded this modal with README, Architecture, and Release Notes tabs.</li></ul></section>
